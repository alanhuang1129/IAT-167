class Player extends Entity {
  ...
  void detectSwing(Entity other) {
    ...
    if (dist(pickaxeSwingHitboxCenter.x, pickaxeSwingHitboxCenter.y, other.pos.x, other.pos.y) < pickaxeSwingHitboxSize && pickaxeReset == false && mouseReleased == true) { //pickaxeReset and mouseReleased are conditions of swinging motion
      //println("detected");
      other.health -= 3;
    }
  }
  void detectSwing(Rock r) {
    if (dist(pickaxeSwingHitboxCenter.x, pickaxeSwingHitboxCenter.y, r.pos.x, r.pos.y) < pickaxeSwingHitboxSize && pickaxeReset == false && mouseReleased == true) {
      r.health -= 3;
    }
  }
